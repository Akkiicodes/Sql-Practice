...
SELECT id, account_id, (standard_amt_usd/standard_qty) AS unit_price_std_ppr
FROM orders
LIMIT 10;
...
SELECT id, account_id, poster_amt_usd*100/(standard_amt_usd + gloss_amt_usd + poster_amt_usd ) AS perc_rev_of_poster_ppr
FROM orders
LIMIT 10;
...
--Use the accounts table to find the account name, primary_poc, and sales_rep_id for (and not) Walmart, Target, and Nordstrom .
...
--Use the web_events table to find all information regarding individuals who were contacted via (and not) the channel of organic or adwords.
